...
Restaurant::Restaurant(ifstream & inn)  {
    std::getline(inn, adresse);
    inn >> telefon >> antStjerner >> antallPlasser >> antallBesok; inn.ignore();
}
...
void Restaurant::lesData()  {
    std::cout << "\nAdresse : "; std::getline(std::cin, adresse);
    telefon = lesInt("Skriv Tlf ", 10000000, 99999999);
    antStjerner = lesInt("Skriv Stjerner ", 0, MAXSTJERNER);
    antallPlasser = lesInt("Antall Plasser ", 0, 999);
}
...
void Restaurant::skrivData() const  {
    ...
    std::cout << "Antall Stjerner : " << antStjerner << std::endl;
    std::cout << "Antall Plasser : " << antallPlasser << std::endl;
    std::cout << "Antall Besok : " << antallBesok << std::endl;
}
...
By::By(ifstream & inn)  {
    ...
    for (int i = 0; i < antall; i++) {
        std::getline(inn, res_navn);
        restaurantene[res_navn] = new Restaurant(inn);
    }
}
 ...
 */
By::~By()  {
    for (auto & [nvn, res] : restaurantene)
        delete res;
    restaurantene.clear();

    std::cout << "\nDeleted By : " << navn << std::endl;
}
...
void By::leggInnNyRestaurant()  {
    std::string navn;
    std::cout << "Skriv navn paa Restaurant : "; std::getline(std::cin, navn);

    auto it = restaurantene.find(navn);
    if (it != restaurantene.end()) {
        std::cout << "Restuarant Finnes Allerede\n";
        return;
    }

    restaurantene[navn] = new Restaurant();
    restaurantene[navn]->lesData();
}
 ...
 */
void By::lesData()  {
    std::cout << "Skriv land : "; std::getline(std::cin, land);
}
...
void By::skrivAlt() const  {
    ...
    std::cout << "Antall Restauranter : " << restaurantene.size() << "\n\n";

    for (const auto & [nvn, res] : restaurantene) {
        std::cout << "Navn : " << nvn << std::endl;
        res->skrivData();
    }
    std::cout << std::endl;
}
...
void By::skrivData()  const  {
    std::cout << "Navn : " << navn << std::endl;
    std::cout << "Land : " << land << std::endl;
    std::cout << "Antall Restauranter : " << restaurantene.size() << std::endl;

    for (const auto & val : restaurantene)
        std::cout << "Restaurant navn : " << val.first << std::endl;
    std::cout << std::endl;
}
...
void By::skrivData(const int antallS) const  {
    for (const auto & [nvn, res] : restaurantene)
        if (res->hentAntStjerner() >= antallS)
            res->skrivData();
}
...
void By::skrivData(const string nvn)  const  {
    std::for_each(restaurantene.begin(), restaurantene.end(), [&](const auto &pair)
    { if (pair.first.find(nvn) != std::string::npos) std::cout << pair.first << std::endl; });
}
...
int finnBy(const string nvn)  {
    auto it = std::find_if(gByene.begin(), gByene.end(), [&](const auto & val)
    { return val->hentID() == nvn; });

    if (it == gByene.end())
        return -1;

    return std::distance(gByene.begin(), it);
}
 ...
 */
void lesFraFil()  {
    std::ifstream inn(R"(C:\Users\Tharald\CLionProjects\Cpp\Exams\S22\Restauranter.dta)");
    if (!inn) {
        std::cout << "\nFant ikke filen\n";
        return;
    }
...
void nyBy()  {
    std::string navn;
    std::cout << "\nSkriv navn : "; std::getline(std::cin, navn);

    int index = finnBy(navn);
    if (index == -1) {
        std::cout << "Fant ikke by : " << navn << std::endl;
        return;
    }

    By* nyBy = new By(navn);
    nyBy->lesData();
    gByene.push_back(nyBy);
}
...
void nyRestaurant()  {
    ...
    gByene[index]->leggInnNyRestaurant();
}
...
void skrivMedStjerner()  {
    int antall = lesInt("\nHvor mange stjerner ", 0, MAXSTJERNER);

    for (const auto & val : gByene) {
        val->skrivData(antall);
    }
}
...
void skrivNavngitte()  {
    std::string navn;
    std::cout << "\nSkriv delvis navn paa resturant : "; std::getline(std::cin, navn);
